...
function getSubscriptionState($user): stdClass {
    ...
    try {
        ...
        $result->registration = mysqli_fetch_object($connection->query("select * from activity_registration where user_id = '$user->id' and event_id = '$activity_id' and payment_id is not null and status not in ('canceled', 'expired', 'failed') order by date desc limit 1"));
